import fs from "fs";
import { camelize, paginate } from "./utils";

const loadOrBuildCache = async (path, fetcher) => {
  let cachedData;

  try {
    cachedData = JSON.parse(fs.readFileSync(path, "utf8"));
  } catch (error) {
...
export async function getPaginatedProducts(page = 1, perPage = 15) {
  const allProducts = await getAllProducts();
  return paginate(allProducts, page, perPage);
}
    ...
    if (hasSubCategories) {
      ...
      const subcategories = cat["Children"]
        ...
        .map((subCategory) => ({
          ...
          href: `/products/${code}/${camelize(subCategory["Name"])}`,
