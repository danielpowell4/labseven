...
async function fetchAllProducts() {
  ...
  while (fetchedProductIndex < lastProductIndex) {
    ...
    const formattedProducts = parsedProducts.map((product) => {
      const manufacturerSkuCode = `${product.Manufacturer}-${product.ManufacturerSku}`.toLowerCase();
      const defaultStyle = (product.Sides || [])[0]; // first

      return {
        ...product,
        manufacturerSkuCode,
        defaultHref: defaultStyle ? `/products/${manufacturerSkuCode}/${camelize(defaultStyle.Name)}` : "",
        Styles: (product.Styles || []).map((style) => {
          ...
          return {
            ...style,
            nameCode,
            href: `/products/${manufacturerSkuCode}/${nameCode}`,
            hasMainImage,
            mainImageUrl: hasMainImage ? (IMAGE_PREFIX + style.ImageFilePath_Front) : "",
