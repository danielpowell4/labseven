...
const MEGA_BYTES_IN_BYTES = 1024 * 1024;
const UPLOAD_FILE_SIZE_LIMIT = 150 * MEGA_BYTES_IN_BYTES; // 150 Mb

const fileSizeInMegabytes = (file) => {
  const asMb = (file?.size || 0) / MEGA_BYTES_IN_BYTES;
  return asMb.toFixed(2) + " MB";
};
...
const FALLBACK_NAME = "Name Missing When Upload Started";
...
const uploadFile = async (dropbox, path, file) => {
  if (file.size < UPLOAD_FILE_SIZE_LIMIT) {
    ...
    return dropbox.filesUpload({ path: path + file.name, contents: file });
  } else {
...
const useFileUpload = (name, addAttachment) => {
  ...
  const onDrop = React.useCallback(
    async (acceptedFiles) => {
      ...
      const nameSelector =
        name && name.startsWith("SELECTOR:") && name.split("SELECTOR:")[1];
      let folderName = nameSelector
      ...
      try {
        for (let i = 0; i < acceptedFiles.length; i++) {
          ...
          addAttachment({
            dropboxFolder: UPLOAD_PATH,
            name: file.name,
            size: fileSizeInMegabytes(file),
          });
        }
        // all complete!
        setData({
        ...
        setTimeout(() => setData({ state: "readyForUpload" }), 5000);
      } catch (error) {
        console.error(error);
        setData({
          state: "error",
