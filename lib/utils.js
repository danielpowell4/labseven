...
export const applyTokens = (activeRow, tokenRes) => {
  activeRow.token = tokenRes.access_token;

  // refresh token is only returned on initial auth (not refresh)
  if (tokenRes.refresh_token) activeRow.refreshToken = tokenRes.refresh_token;

  // token expires in ~4 hours
  const expiresAt = Date.now() + tokenRes.expires_in * 1000; // ms
  activeRow.expiresAround = new Date(expiresAt).toISOString();
  activeRow.lastUpdated = new Date().toISOString();
};
